...
function verExistencias(XML,producto, requerido){
...
var checarSaldo={
       ...
       that.calculate(docXML,isOA);
		...
		calculate:function(docXML,isOA){
		    var result=[];
		    var that= this;
		    if(isOA){
		...
		docXML.find("Nivel:contains('01')").each(function(){
		    var $that=$(this);
		    var $padre= $that.closest("Componente");
		    ...
		    var codProducto  = $padre.find("Producto").text(),
                    ...
                    var valueSearch=codigoPadre;


                   var id=$("input[name^='clave_prod___']").filter( "[value^='"+valueSearch+"']").attr("id");
                    var number= id.replace("clave_prod___","");
                    var lotesEnlinea="";
                    for(var i in lotesTotales){
                        
                        lotesEnlinea+= lotesTotales[i].lotes;
                    ...
                    $("#cant_surtida___"+number).val(UM);
                

                    
		  
		     console.log("cod"+" "+codProducto+" "+descProducto +" formula "+formula+" "+UM+" saldo "+saldo+" "+codigoPadre+" "+lotes+" "+requerido +" reqCalculado"+requeridoCalculado +" "+lotesTotales +" "+requeridoCalculado+" lotes"+JSON.stringify(lotesTotales)+" esmayor"+(saldo>=requeridoCalculado) );
